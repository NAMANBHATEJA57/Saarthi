    ...
    <div className="rounded-xl border border-[hsl(var(--hairline))] bg-[hsl(var(--surface))] p-4 transition-all hover:border-[hsl(var(--ink-muted))] flex flex-col justify-between group">
      <div>
        <div className="flex justify-between items-start mb-2">
          ...
          {/* We check both snake_case and camelCase to handle raw scraped jobs OR db records */}
          {(job.company_logo || job.companyLogo) && (
            <img src={job.company_logo || job.companyLogo} alt={job.company} className="w-10 h-10 rounded-md object-contain bg-white border border-[hsl(var(--hairline))]" />
          )}
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span>
              {(job.salary_min || job.salaryMin) && (job.salary_max || job.salaryMax)
                ? `${job.salary_currency || job.salaryCurrency || '$'}${job.salary_min || job.salaryMin} - ${job.salary_max || job.salaryMax} / ${job.salary_interval || job.salaryInterval || 'yr'}`
                : "Salary not listed"}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Briefcase className="w-3.5 h-3.5" />
            <span className="capitalize">{job.job_type || job.jobType || "Full-time"}</span>
          </div>
          <div className="flex items-center gap-2">
            <Calendar className="w-3.5 h-3.5" />
            <span>{(job.date_posted || job.datePosted) ? new Date(job.date_posted || job.datePosted).toLocaleDateString() : "Recently"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-between gap-2 border-t border-[hsl(var(--hairline))] pt-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {(job.job_url || job.jobUrl) && (
            <a href={job.job_url || job.jobUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center font-medium transition-all duration-200 ease-out active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--primary))] focus-visible:ring-offset-2 focus-visible:ring-offset-[hsl(var(--background))] disabled:opacity-50 disabled:pointer-events-none bg-[hsl(var(--primary))] text-white hover:bg-[hsl(var(--primary-active))] rounded-full px-4 py-1.5 text-xs shadow-sm h-8">
